...
export class GetInfoService {
  ...
  public onLike(data: {username: string, post_id: number}, accessToken: string) {
    let headers = new HttpHeaders().set('Content-Type', 'application/json').set('Authorization', 'Bearer ' + accessToken);
    return this.http.post(this.baseUrl + 'like_post', data, {headers: headers});
  }

  public getNotifications(data: {username: string}, accessToken: string) {
    let headers = new HttpHeaders().set('Content-Type', 'application/json').set('Authorization', 'Bearer ' + accessToken);
    return this.http.post(this.baseUrl + 'get_notifications', data, {headers: headers});
  }
}
