...
def check_friends(user1, user2):
	...
	query = """SELECT user1 from friends where (user2 = (SELECT id from users where username = "%s") and 
	user1 = (SELECT id from users where username = "%s")) or (user1 = (SELECT id from users where username = "%s") and 
	user2 = (SELECT id from users where username = "%s"))""" % (user1, user2, user1, user2)
	cur.execute(query)
	rv = cur.fetchone()
...
def getPosts():
	...
	where poster_id = (SELECT id from users where username = '%s') and id=poster_id""" % (request.json['forUsername'])
	if request.json['fromUsername'] == request.json['forUsername']:
		...
		if not fr:
			query = """SELECT post_id, poster_id, name, post_date, liked, public, post, image from posts, info 
			where poster_id = (SELECT id from users where username = '%s') and id=poster_id and public = 1""" % (request.json['forUsername'])
	cur.execute(query)
	rv = cur.fetchall()
	if not rv:
...
def getFriendPosts():
	...
	for result in rv:
		query = """SELECT post_id, poster_id, name, post_date, liked, public, post, image from posts, info 
		where poster_id = '%s' and id=poster_id""" % (result)
		cur.execute(query)
		rv1 = cur.fetchall()
		row_headers=[x[0] for x in cur.description] #this will extract row headers
		for i in rv1:
